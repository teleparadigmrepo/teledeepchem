...
from layers import ProtacGraphConv,SmilesNet
...
class DeepPROTAC(TorchModel):
    def __init__(self, **kwargs):
        ...
        ligase_ligand = ProtacGraphConv(num_embeddings=10)  
        ligase_pocket = ProtacGraphConv(num_embeddings=5)   
        target_ligand = ProtacGraphConv(num_embeddings=10)  
        target_pocket = ProtacGraphConv(num_embeddings=5)   
    ...
    def predict(self, dataset, transformers=None, output_types=None):
        ...
        with torch.no_grad():
            for batch in dataset:
                ...
                if isinstance(batch, tuple) and len(batch) == 3:
                    inputs,_,  = batch  # Unpack if in tuple format
                else:
